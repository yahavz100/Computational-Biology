...
def calculate_fitness(decrypted_text: str, english_words: set, letter_frequencies: dict, letter_pair_frequencies: dict) -> float:
    """
    ...
    for letter, freq in letter_freqs.items():
        expected_freq = letter_frequencies.get(letter, 0)
        fitness += abs(expected_freq - freq)
    for pair, freq in letter_pair_freqs.items():
        expected_freq = letter_pair_frequencies.get(pair, 0)
        fitness += abs(expected_freq - freq)
...
def optimize_key_fitness(ciphertext, encrypted_letter_freq, encrypted_letter_pair_freq, english_word_set,
    ...
    def fitness_function(decryption_key):
        ...
        return calculate_fitness(plaintext, encrypted_letter_freq, encrypted_letter_pair_freq, english_word_set)
